...
export default function AuthPage() {
  ...
  const initGoogleGSI = useCallback(() => {
    if (_gsiInitialized || typeof google === 'undefined') return;
        ...
        {mode === 'login' ? (
          <form className="auth-form" onSubmit={handleLogin} key="login">
            ...
            <div className="auth-social-buttons">
              <button type="button" className="btn btn-social btn-gmail" onClick={() => {
                setSocialErr('');
                initGoogleGSI();
                if (google?.accounts?.id) google.accounts.id.prompt();
                else setSocialErr('Google Sign-In not loaded yet. Try again.');
              }}>
