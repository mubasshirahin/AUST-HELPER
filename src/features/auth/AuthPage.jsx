...
export default function AuthPage() {
  ...
  useEffect(() => {
    if (typeof window.google === 'undefined') {
      ...
      script.onload = () => {
        ...
        initGoogleGSI();
        // Render the Google Sign-In button once GIS is loaded
        if (googleBtnRef.current && window.google?.accounts?.id) {
          window.google.accounts.id.renderButton(googleBtnRef.current, {
            type: 'standard', theme: 'outline', size: 'large', shape: 'rectangular',
            width: Math.min(googleBtnRef.current.parentElement?.offsetWidth || 280, 400),
            text: 'signin_with',
            logo_alignment: 'left',
          });
        }
  ...
  const handleSocialLogin = (provider) => {
    if (provider === 'gmail') {
      // Button rendered by GIS — just clear any error
      setSocialErr('');
      return;
    }
    const accounts = getAllAccounts().filter((a) => a.id !== 'guest');
        ...
        {mode === 'login' ? (
          <form className="auth-form" onSubmit={handleLogin} key="login">
            ...
            <div className="auth-social-buttons">
              <div className="btn btn-social btn-gmail" style={{ position: 'relative', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'var(--sp-2)', pointerEvents: 'none', padding: 'var(--sp-2) var(--sp-4)' }}>
                  <svg viewBox="0 0 24 24" width="16" height="16" style={{ flexShrink: 0 }}><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                  <span>Login with Google</span>
                </div>
                <div ref={googleBtnRef} style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer', zIndex: 1 }} />
              </div>
